...
public class MusicianControllerUnitTests {
    @Autowired
    ...
    // GET : Musician
    @Test
    public void whenGetMusicianByAll_thenReturnJsonMusician() throws Exception {
        ...
        given(musicianRepository.findAll()).willReturn(musicianList);
    ...
    public void whenGetMusicianById_thenReturnJsonMusician() throws Exception {
        ...
        given(musicianRepository.findMusicianById(musician.getId())).willReturn(musician);

        mockMvc.perform(get("/musician/{id}", musician.getId()))
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    public void whenGetMusicianByName_thenReturnJsonMusician() throws Exception {
        ...
        given(musicianRepository.findMusicianByName("Svend Asmussen")).willReturn(musician);
    ...
    public void whenGetMusicianByYear_thenReturnJsonMusician() throws Exception {
        ...
        given(musicianRepository.findAllByFromYear(2000)).willReturn(musicianList);
    ...
    public void whenGetMusicianInstrumentById_thenReturnJsonMusician() throws Exception {
        ...
        Musician musician1 = new Musician("Svend Asmussen", 1916, 2017,instrument1);
        musicianList.add(musician1);
        given(musicianRepository.findMusicianByInstrument_Id(musicianList.get(0).getId())).willReturn(musicianList);
        mockMvc.perform(get("/musician/instrument/{id}", musicianList.get(0).getId()))
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].name", is("Svend Asmussen")))
                .andExpect(jsonPath("$[0].yearOfBirth", is(1916)))
    ...
    public void whenGetMusicianInstrumentByName_thenReturnJsonMusician() throws Exception {
        List<Musician> musicianList = new ArrayList<>();
        Instrument instrument = new Instrument("Trumpet", "If you think the brass family got its name because the instruments are made of brass, you're right! This family of instruments can play louder than any other in the orchestra and can also be heard from far away. Although their early ancestors are known to have been made of wood, tusks, animal horns or shells, today's modern instruments are made entirely of brass. Brass instruments are essentially very long pipes that widen at their ends into a bell-like shape. The pipes have been curved and twisted into different shapes to make them easier to hold and play.", "1980", "Brass");
        Musician musician = new Musician("Irving Fields", 1915, 2016,instrument);
        instrument.setMusicians(Collections.singletonList(musician));

        musicianList.add(musician);

        given(musicianRepository.findMusicianByInstrument_Name("Trumpet")).willReturn(musicianList);
